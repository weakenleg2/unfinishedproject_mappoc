...
class MLPBase(nn.Module):
    ...
    def forward(self, ob, option = None, stochastic=True):
        ...
        return ac, self.vpred, mean, logstd

    def act(self, stochastic, ob, option):
        # print(option,type(option))
        ac, vpred, mean,logstd = self.forward(ob, option, stochastic)
        # print(f"ac:{ac[0]}")
    ...
    def get_vpred(self, ob, option):
        # print(option,type(option))
        ac, vpred, mean,logstd = self.forward(ob, option)
        return vpred[0].detach().cpu().numpy()
    def evaluate_actions(self, states, actions):
        """
        Evaluate the actions taken by the policy in given states.

        Args:
        - states: Tensor representing a batch of states.
        - actions: Tensor representing a batch of actions.

        Returns:
        - log_probs: Log probabilities of the actions taken.
        - state_values: Estimated value of the states.
        - entropy: Entropy of the policy for the given states.
        """
        states = prepare_observation(states)
        if states.shape[0]==1:
            states_q = torch.clamp((states), -10.0, 10.0)
        else:
            self.states_rms_q.update(states)
            
            states_q = torch.clamp((states - self.states_rms_q.mean) / self.states_rms_q.std, -10.0, 10.0)
        
        # Process states through the network to get policy and value outputs
        # Assuming pi_net gives policy output and q_net* gives value output
        policy_output = self.pi_net(states_q)
        # print(policy_output.shape)
        
        # Calculate action probabilities, log probabilities, and entropy
        # if self.ac_space.__class__.__name__ == "Box" and self.gaussian_fixed_var:
        log_std = nn.Parameter(torch.zeros(1, self.ac_space_dim))
        std = log_std.exp()

        pd = Normal(policy_output, policy_output * 0.0 + std)
        ...
        # Assuming option=None and using q_net1 for value prediction (change as per your design)
        vpred = self.q_net1(states)[:, 0]

        return log_probs, vpred, entropy
